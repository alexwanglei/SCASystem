...
public class SamplePort extends Port{
	...
	public String revPortMsg(IOput i, int n){
		StringBuffer code = new StringBuffer();
		int num = i.getId();
		code.append("		READ_SAMPLING_MESSAGE(samplePort_"+super.getId()+", (MESSAGE_ADDR_TYPE)&"+i.getName().replaceAll(" ", "")+", &lendata"+n+", &validity"+num+", &retCode);\n" +
	...
	public String sendPortMsg(IOput o, int n){
		StringBuffer code = new StringBuffer();
		code.append("		 WRITE_SAMPLING_MESSAGE(samplePort_"+super.getId()+", (MESSAGE_ADDR_TYPE)&"+o.getName().replaceAll(" ", "")+", lenMsgData"+n+", &retCode);\n" +
